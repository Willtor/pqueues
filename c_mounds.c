...
c_mound_pq_t* c_mound_pq_create(size_t size) {
  ...
  atomic_store_explicit(&mound_pqueue->depth, 1, memory_order_relaxed);
  return mound_pqueue;
}
...
static mound_node_t* lock(c_mound_pq_t *pqueue, size_t i) {
  ...
  pthread_spin_lock(&mound->lock);
  return mound;
}

static void unlock(c_mound_pq_t *pqueue, size_t i) {
  mound_node_t *mound = &pqueue->tree[i];
  pthread_spin_unlock(&mound->lock);
}
...
static size_t linear_search(c_mound_pq_t *pqueue, uintmax_t leaf, uintmax_t root, int64_t priority) {
  ...
  for(uintmax_t  parent = leaf / 2; parent != 0; last_index = parent, parent /= 2) {
    list_node_t *list = atomic_load_explicit(&pqueue->tree[parent].list, memory_order_consume);
...
static uintmax_t find_insert_point(uint64_t *seed, c_mound_pq_t *pqueue, int64_t priority) {
  while(true) {
    ...
    for(uintmax_t i = 0; i < THRESHOLD; i++) {
      ...
      list_node_t *list = atomic_load_explicit(&pqueue->tree[random_leaf].list, memory_order_consume);
...
static void moundify(c_mound_pq_t *pqueue, size_t i) {
  while(true) {
    list_node_t *current = atomic_load_explicit(&pqueue->tree[i].list, memory_order_consume);
    uintmax_t depth = atomic_load_explicit(&pqueue->depth, memory_order_relaxed);
    if(is_leaf(depth, i)) {
      unlock(pqueue, i);
      return;
    ...
    list_node_t *left = atomic_load_explicit(&left_mound->list, memory_order_consume);
    list_node_t *right = atomic_load_explicit(&right_mound->list, memory_order_consume);    
    ...
    if(left_val <= right_val && left_val < current_val) {
      ...
      atomic_store_explicit(&pqueue->tree[i].list, left, memory_order_release);
      unlock(pqueue, i);
      atomic_store_explicit(&pqueue->tree[left_index].list, current, memory_order_release);
      moundify(pqueue, left_index);
    } else if(right_val < left_val && right_val < current_val) {
      unlock(pqueue, left_index);
      atomic_store_explicit(&pqueue->tree[i].list, right, memory_order_release);
      unlock(pqueue, i);
      atomic_store_explicit(&pqueue->tree[right_index].list, current, memory_order_release);
...
int c_mound_pq_add(uint64_t *seed, c_mound_pq_t * pqueue, int64_t priority) {
  while(true) {
    ...
    if(insertion_point == ROOT) {
      ...
      list_node_t *list = atomic_load_explicit(&root->list, memory_order_consume);
      if(get_val(list) >= priority) {
        list_node_t *new_node = list_node_create(list, priority);
        atomic_store_explicit(&root->list, new_node, memory_order_release);
        unlock(pqueue, ROOT);
        return true;
      }
      unlock(pqueue, ROOT);
    ...
    list_node_t *parent_list = atomic_load_explicit(&parent->list, memory_order_consume);
    list_node_t *child_list = atomic_load_explicit(&child->list, memory_order_consume);
    if(get_val(child_list) >= priority && get_val(parent_list) <= priority) {
      list_node_t *new_node = list_node_create(child_list, priority);
      atomic_store_explicit(&child->list, new_node, memory_order_release);
...
int c_mound_pq_leaky_pop_min(c_mound_pq_t * pqueue) {
  ...
  atomic_store_explicit(&root->list, list->next, memory_order_release);
